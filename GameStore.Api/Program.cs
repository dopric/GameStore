...
app.MapGet("/api/games/{id}", (int id) => games.FirstOrDefault(g => g.Id == id) is Game game ? Results.Ok(game) : Results.NotFound())
.WithName(GetGameByIdName);

app.MapPost("/api/games", (Game game) =>
{
    games.Add(game);
...
app.MapPut("/api/games/{id}", (int id, Game game) =>
{
